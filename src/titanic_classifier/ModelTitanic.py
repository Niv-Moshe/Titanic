...
class ModelTitanic:
    ...
    def get_pipeline(self, model=None, model_name: str = 'model', train: bool = False) -> Pipeline:
        ...
        pipeline.steps.append(['feature_select',
                               RFECV(estimator=LogisticRegression(), step=1, cv=5, scoring='f1')])
        pipeline.set_output(transform="pandas")
    ...
    def perform_model_selection(self, ):
        """
        This method performs model selection, first creating the preprocessed train dataframe once at max as it is the
        same for all models (and actually not necessary for catboost/lightgbm model).
        Returns:
